...
public class BbsController {
	...
	/*
	 * @Autowired : 스프링 설정파일에서 이미 생성된 빈을 자동으로 주입받고 싶을때 사용한다. 타입을 기반으로 자동주입되며, 만약 해당
	 * 타입의 빈이 존재하지 않으면 에러가 발생되어 서버를 시작할 수 없다. -생성자,멤버변수,메소드(setter)에 적용가능함. -타입을 이용해
	 * 자동으로 프로퍼티 값을 설정함. -해당 어노테이션은 멤버변수에만 적용할 수 있다. 함수 내의 지역변수에서는 사용할 수 없다. -타입을 통해
	 * 자동으로 설정되므로 같은 타입이 2개 이상 존재하면 예외가 발생됨.
	 */

	/*
	 * 멤버변수로 선언하여 클래스에서 전역적으로 사용할 수 있다. 해당 클래스의 모든 command객체는 해당 인터페이스를 구현하여 정의한다.
	 */
	...
	public String list(Model model, HttpServletRequest req) {
		/*
		 * 사용자로부터 받은 모든 요청은 request객체에 저장되고, 이를 ListCommand객체로 전달하기 위해 model에 저장한 후
		 * 매개변수로 전달한다.
		 */
	...
	public String writeAction(Model model, HttpServletRequest req, SpringBbsDTO springBbsDTO) {
		...
		/*
		 * 글쓰기 페이지에서 전송된 모든 폼값을 SpringBbsDTO객체가 한번에 받는다. Spring에서는 커맨드 객체를 통해 이와 같이 할 수
		 * 있다.
		 */
		...
		/*
		 * redirect : 이동할페이지경로(요청명)와 같이 하면 뷰를 호출하지 않고 페이지 이동이 된다.
		 */
	...
	public String passwordAction(Model model, HttpServletRequest req) {
		...
		if (rowExist <= 0) {
			/*
			 * 게시물의 일련번호가 0이하의 값을 가질 수 없으므로 검증실패로 판단할 수 있다.
			 */
	...
	public String editAction(HttpServletRequest req, Model model, SpringBbsDTO springBbsDTO) {
		...
		/*
		 * 뷰로 이동 시 redirect를 이용하면 해당 페이지로 리다이렉트 된다. 이때 파라미터가 필요하다면 model에 저장하기만 하면 자동으로
		 * 쿼리스트링이 추가된다.
		 */
		model.addAttribute("idx", req.getParameter("idx"));
		model.addAttribute("nowPage", req.getParameter("nowPage"));
